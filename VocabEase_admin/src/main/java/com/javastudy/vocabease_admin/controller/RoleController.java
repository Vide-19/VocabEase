...
import com.javastudy.vocabease_common.entity.dto.SaveRoleMenuDTO;
...
public class RoleController extends ABaseController {
    ...
    public ResponseVO<PaginationResultVO<Role>> loadRoles(RoleQuery query) {
    ...
    public ResponseVO<Void> saveRole(@RequestBody @VerifyParam Role role) {
        this.roleService.savaRole(role, role.getMenuIdx());
    ...
    @GlobalInterceptor(permissionCode = PermissionCodeEnum.SETTINGS_ROLE_EDIT)
    public ResponseVO<Void> saveRole2Menu(@RequestBody @VerifyParam SaveRoleMenuDTO dto) {
        roleService.saveRole2Menu(dto.getRoleId(), dto.getMenuIds());
        return getSuccessResponseVO(null);
    }
    ...
    @GlobalInterceptor(permissionCode = PermissionCodeEnum.SETTINGS_ROLE_LIST)
    public ResponseVO<PaginationResultVO<Role>> loadRolesList() {
