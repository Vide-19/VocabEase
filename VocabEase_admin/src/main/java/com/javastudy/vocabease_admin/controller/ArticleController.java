...
public class ArticleController extends ABaseController {
    ...
    public ResponseVO<PaginationResultVO<Article>> loadDataList(ArticleQuery query) {
    ...
    @GlobalInterceptor(permissionCode = PermissionCodeEnum.ARTICLE_POST)
    public ResponseVO<Void> topArticle(@VerifyParam(required = true) String articleIds,
                                       SessionStatus sessionStatus) {
        this.articleService.updateArticleStatus(articleIds, PostStatusEnum.IS_TOP.getStatus());
        return getSuccessResponseVO(null);
    }
